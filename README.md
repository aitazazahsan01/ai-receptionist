...
| Doc | What's in it |
|---|---|
| [docs/01-CONCEPTS.md](docs/01-CONCEPTS.md) | Every unfamiliar term (WebRTC, LiveKit, VAD, streaming, event-driven architecture, function calling...) explained from first principles, with why it's needed here. |
| [docs/02-ARCHITECTURE.md](docs/02-ARCHITECTURE.md) | The actual system design: components, data flow diagrams, database schema, tech stack decisions and their alternatives. |
| [docs/03-FREE-TIER-STACK.md](docs/03-FREE-TIER-STACK.md) | Exactly which free/trial accounts to sign up for, their real limits (verified August 2026), and how to sequence them so you don't burn trial credit before you're ready. |
| [docs/04-IMPLEMENTATION-PLAN.md](docs/04-IMPLEMENTATION-PLAN.md) | A phase-by-phase build plan from "empty folder" to "working demo," with concrete deliverables and concept call-outs per phase. |
| [docs/05-SETUP-AND-RUN.md](docs/05-SETUP-AND-RUN.md) | The mechanical checklist for actually running this end-to-end on a machine with Docker: account signups, `.env` setup, LiveKit/Twilio wiring, and how to start everything. |
